...
TEST_CASE("remove_dc") {
    ...
    auto f = [&](double x) { return a * x + b; };

    auto f_ref = [&](double x) { return a * (x - n / 2); };
...
TEST_CASE("linear_regression_with_index") {
    ...
    SECTION("Array") {
        Array<double, 3> ys{};
        for_each_with_index_mut([&](int i, double& x) { x = f(i); },
                                ys);
...
TEST_CASE("detrend") {
    ...
    auto f1 = [&](double x) { return a * x + b; };

    auto f2 = [n](double x) { return sin(2 * M_PI * x / n); };
    ...
    SECTION("std::array") {
        ...
        auto ys = map(op_add<double>, ys1, ys2);
    ...
    SECTION("Vector") {
        ...
        const auto ys = map(op_add<double>, ys1, ys2);
