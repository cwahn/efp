...
#define STAMP256(n, x) \
    ...
    class EnumBase {
      public:
        ...
        template<uint8_t n>
        auto get() -> EnableIf<lt_v(n, sizeof...(As)), PackAt<n, As...>> {
            if (_index != n) {
                throw std::runtime_error("Wrong variant index");
            }
        ...
        template<uint8_t n>
        auto move() const -> EnableIf<lt_v(n, sizeof...(As)), PackAt<n, As...> const&&> {
            if (_index != n) {
                throw std::runtime_error("Wrong variant index");
            }
        ...
        template<uint8_t n>
        auto move() -> EnableIf<lt_v(n, sizeof...(As)), PackAt<n, As...>&&> {
            if (_index != n) {
                throw std::runtime_error("Wrong variant index");
            }
        ...
        template<typename... Fs>
        auto match(const Fs&... fs) const -> EnableIf<
            AreAllRelevantBranchs<Fs...>::value && IsExhaustive<Fs...>::value
                && IsWellFormed<Fs...>::value,
            Common<Return<Fs>...>> {
            return detail::MatchImpl<detail::power_2_ceiling(sizeof...(As)), As...>::impl(
                Overloaded<MatchBranch<Fs>...> {fs...},
                this
            );
        }
