...
namespace efp
{
    ...
    double sse(const SeqA &as, const SeqB &bs)
    {
    ...
    template <typename SeqA, typename SeqB>
    double mse(const SeqA &as, const SeqB &bs)
    {
        const auto square_error = [](Element_t<SeqA> a, Element_t<SeqB> b)
        {
            const auto error = a - b;
    ...
    template <typename SeqA>
    double standard_deviation(const SeqA &as)
    {
        return sqrt(variance(as));
    }

    template <typename SeqA, typename SeqB>
    double covariance(const SeqA &as, const SeqB &bs)
    {
        const double a_mean = mean(as);
    ...
    template <typename SeqA>
    MapSequence_t<double, SeqA> remove_dc(const SeqA &as)
    {
        const double a_mean = mean(as);
    ...
    template <typename SeqA, typename SeqB>
    std::tuple<double, double> linear_regression(const SeqA &as, const SeqB &bs)
    {
        const double a_mean = mean(as);
        const double b_mean = mean(bs);
    ...
    template <typename SeqA>
    std::tuple<double, double> linear_regression_with_index(const SeqA &as)
    {
    ...
    template <typename SeqA>
    MapSequence_t<double, SeqA> detrend(const SeqA &as)
    {
