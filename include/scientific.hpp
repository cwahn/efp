...
R sse(const As& as, const Bs& bs) {
    const auto square_error = [](const Element<As>& a, const Element<Bs>& b) { return square(a - b); };
...
R covariance(const As& as, const Bs& bs) {
    ...
    const auto covar = [&](const Element<As>& a, const Element<Bs>& b) { return (a - as_mean) * (b - bs_mean); };
...
Tuple<R, R> linear_regression_with_index(const As& as) {
    ...
    const auto get_ss_ia = [&](const int& i, const Element<As>& a) { return (i - mean_is) * ((R)a - as_mean); };
...
NAryReturn<R, As> detrend(const As& as) {
    ...
    const auto detrend_elem = [&](const Element<As>& i, const Element<As>& x) { return (R)x - (beta_1 * (R)i + beta_2); };
